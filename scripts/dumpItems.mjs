...
const items = await page.evaluate(
  async ({ b64, pageNum }) => {
    ...
    const mod = await import(/* @vite-ignore */ "/node_modules/pdfjs-dist/build/pdf.mjs");
    if (!mod.GlobalWorkerOptions.workerSrc) {
      mod.GlobalWorkerOptions.workerSrc = "/node_modules/pdfjs-dist/build/pdf.worker.mjs";
    }
...
console.log("idx | text                  | x       y      w     h    | font");
...
items.forEach((it, i) => {
  ...
  const text = it.str === "" ? "(empty)" : `"${it.str}"`.padEnd(22, " ").slice(0, 22);
  const codes = !it.str
    ? ""
    : "[" +
      Array.from(it.str)
        ...
        .join(" ") +
      "]";
  const xyhw = `${String(it.x).padStart(7)} ${String(it.y).padStart(6)} ${String(it.w).padStart(
    5,
  )} ${String(it.h).padStart(4)}`;
  console.log(`${String(i).padStart(3)} | ${text} | ${xyhw} | ${it.font} ${codes}`);
});
